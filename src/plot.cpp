...
void PLOT::init(EKF &ekf,GMAP &gmap,LOCKS &locks,bool &running)
{
   ...
   while(!viewer.wasStopped()||ekf.run == true)
    {     
      ...
      locks.ekf_run_mtx.unlock();
        ...
        locks.ReadGlobalMAP_mtx.lock();
          GLOBALmap = gmap.store.GLOBALmap; 
          GLOBALmap_color = gmap.store.GLOBALmap_color;
          path = gmap.store.path;
        locks.ReadGlobalMAP_mtx.unlock();
        count = 0;
      }
      else
      ...
      if(PAR.plot_3D.show_camera_pose == true)
      {
       ...
       if(PAR.plot_3D.show_key_frames == true)
       {
        ...
        { 
          ...
          viz::WTrajectoryFrustums frustums(path, Vec2f(0.889484, 0.523599), 0.1,
                                      viz::Color::yellow());
