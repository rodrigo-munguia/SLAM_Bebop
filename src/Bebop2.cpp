 ...
 *    
 * 
 * Based on ViSP, open source Visual Servoing Platform software.
 *  
 * 
 *
 *****************************************************************************/
...
//-------------------------------------------------------------------------
// Rodrigo M
void vpRobotBebop2::SendAltitude()
{
  
//  ReceivedAltitudeCallback(m_local_altitude);

}

void vpRobotBebop2::SendFrame()
{
...
if (m_videoDecodingStarted) {
    ...
    if (m_bgr_picture->data[0] != NULL) {
      ...
      ReceivedFrameCallback(resized_up);   // fireup callback
...
void vpRobotBebop2::cmdSpeedChange(ARCONTROLLER_DICTIONARY_ELEMENT_t *elementDictionary, vpRobotBebop2 *drone)
{
        ...
        if(drone->Speed_Callback_is_Set== true)
        {
                drone->ReveicedSpeedCallback(speedX,speedY,speedZ);
        }
       ...
       clock_gettime(CLOCK_REALTIME, &tpe); // get the current time
        ...
        {   
            ...
            if (arg != NULL)
            {
               ...
               //drone->m_local_altitude = altitude; 
               //drone->SendAltitude();
               if(drone->Altitude_Callback_is_Set== true)
               {
                drone->ReceivedAltitudeCallback(altitude);
               }
...
{       
        ...
        {   
            ...
            if(drone->Attitude_Callback_is_Set== true)
            {
                drone->ReveicedAttitudeCallback(roll,pitch,yaw);
            } 
