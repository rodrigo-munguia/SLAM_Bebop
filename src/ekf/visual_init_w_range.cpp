...
void init_XYZ_feat_point(arma::vec& x, arma::mat& P, parameters &par,FRAME *frame,cv::KeyPoint kp,cv::Mat Descriptor, vectorFeat& FeatsDATA)
{
    ...
    //---------
    arma::vec::fixed<3> d_v =  depth*m;
    
    arma::vec new_yi = x.subvec(7,9) + d_v;   // compute new Euclidean (XYZ) feature map
    
    //cout << frame->range_type << " " << depth << endl;
    ...
    if( (dp < r_range_image)&&(frame->range < par.sensors.range_sensor.max_range_operation ))
    {
        ...
        {  // for initial features
           sig_d_ini = depth/(par.ekf.sigma_id_range_dem*10); 
        }  
    }
    else
    {   
        ...
        {  // for initial features
        sig_d_ini = depth/(par.ekf.sigma_id_WOrange_dem*10);  
        } 
    }
